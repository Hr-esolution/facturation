        ...
        <div class="col-12">
            <div class="glass-card p-4">
                <div class="d-flex justify-content-between align-items-center">
                    <h1 class="text-white mb-0">Gestion des Clients</h1>
                    <a href="{{ route('clients.create') }}" class="btn-glass btn-primary-glass">
                ...
                <div class="row align-items-center">
                    <div class="col-md-4">
                        <div class="search-box">
                            <input type="text" class="form-control bg-transparent text-white border-0" placeholder="Rechercher un client..." style="background: transparent;">
                        </div>
                    </div>
                    <div class="col-md-8">
                            ...
                            @forelse($clients ?? collect([]) as $client)
                                <tr>
                                    <td>
                                        ...
                                        <small class="text-white/70">{{ $client->prenom }}</small>
                                    </td>
                                    <td>{{ $client->email }}</td>
                                    <td>{{ $client->telephone }}</td>
                                    <td>{{ $client->ville }}</td>
                                    <td>
                                        <span class="badge bg-success" style="background: rgba(144, 190, 109, 0.3); color: white; border: 1px solid rgba(144, 190, 109, 0.5);">
                                            Actif
                                        </span>
                                    </td>
                                    <td class="text-end">
                                        <div class="d-flex gap-2 justify-content-end">
                                            <a href="{{ route('clients.show', $client->id) }}" class="btn-glass btn-sm">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('clients.edit', $client->id) }}" class="btn-glass btn-sm">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('clients.destroy', $client->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn-glass btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce client ?')">
                ...
                <div class="p-3 border-top" style="border-color: rgba(255, 255, 255, 0.1) !important;">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="text-white/70">
                            Affichage de 1 à {{ $clients->count() ?? 0 }} sur {{ $clients->total() ?? $clients->count() ?? 0 }} entrées
                        </div>
                        <nav>
